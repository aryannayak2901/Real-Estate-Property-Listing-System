...
from .models import OtpToken, Property_details, Wishlist
import secrets
import random
...
def profile(request, wishlist_property_id):
    property = Property_details.objects.get(pk=wishlist_property_id)
    wishlist, created = Wishlist.objects.get_or_create(user=request.user)
    wishlist.properties.add(property)
    return redirect('/profile_directly')
    # return render(request, "profile.html")

def profile_directly(request):
    if request.user.is_authenticated:
        wishlist, created = Wishlist.objects.get_or_create(user=request.user)
        properties = wishlist.properties.all()
    else:
        properties = []
    ...
    return render(request, "profile.html", {'properties': properties, 'username': username, 'email': email})
